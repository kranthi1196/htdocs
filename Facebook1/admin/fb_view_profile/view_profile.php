...
<table cellspacing="0">
<?php
	$que_post=mysqli_query($connect, "select * from user_post where user_id=$v_user_id order by post_id desc");
...
<?php 
	...
	<tr style="color:#6D84C4;">
		<td >   </td>
		<?php
		 	$que_status=mysqli_query($connect, "select * from user_post_status where post_id=$postid and user_id=$v_user_id;");
...
<!-- Photos bg-->
    ...
    <?php
	$que_post_img=mysqli_query($connect, "select * from user_post where user_id=$v_user_id and post_pic!='' order by post_id desc");
