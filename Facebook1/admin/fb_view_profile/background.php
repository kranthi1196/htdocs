...
?>
<?php 
	$que_v_user_info=mysqli_query($connect, "select * from users where user_id=$v_user_id");
	...
	$que_view_user_profile_pic=mysqli_query($connect, "select * from user_profile_pic where user_id=$v_user_id");
	$user_profile_pic_data=mysqli_fetch_array($que_view_user_profile_pic);
	$profile_img=$user_profile_pic_data[2];


	$que_user_cover_pic=mysqli_query($connect, "select * from user_cover_pic where user_id=$v_user_id");
...
{
	mysqli_query($connect, "update user_status set status='Offline' where user_id=$v_user_id");
}
...
if(isset($_POST['warning_user']))
{
		...
		mysqli_query($connect, "insert into user_warning values($warning_id,'$warning_txt')");
}
...
if(isset($_POST['file']) && ($_POST['file']=='Upload'))
{
    	...
    	mysqli_query($connect, "update user_profile_pic set image='$img_name' where user_id=$v_user_id;");
...
if(isset($_POST['file1']) && ($_POST['file1']=='Upload'))
{
    	...
    	mysqli_query($connect, "insert into user_cover_pic(user_id,image) values('$v_user_id','$img_name');");
		header("location:view_profile.php?id=".$v_user_id."");
}
...
if(isset($_POST['file2']) && ($_POST['file2']=='Upload'))
{
		...
		mysqli_query($connect, "update user_cover_pic set image='$img_name' where user_id=$v_user_id;");
		header("location:view_profile.php?id=".$v_user_id."");
}
?>
<html>
