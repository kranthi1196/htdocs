<?php
	...
	if(isset($_SESSION['fbuser']))
	{
		...
		$que_user_info=mysqli_query($connect, "select * from users where Email='$user_email'");
		...
		if(isset($_POST['feedback']))
		{
			...
			mysqli_query($connect, "insert into feedback(user_id,feedback_txt,star,Date) values($userid,'$fb_txt','$star','$fb_time')");
		}
		
		if(isset($_POST['delete_feedback']))
		{
			$fb_id=intval($_POST['feedback_id']);
			mysqli_query($connect, "delete from feedback where feedback_id=$fb_id");
		}
...
<?php
		$que_feedback=mysqli_query($connect, "select * from feedback order by feedback_id desc");
?>
...
<?php
	while($feedback_data=mysqli_fetch_array($que_feedback))
	{
		...
		$que_fb_user_info=mysqli_query($connect, "select * from users where user_id=$fb_user_id");
		...
		$que_fb_user_pic=mysqli_query($connect, "select * from user_profile_pic where user_id=$fb_user_id");
