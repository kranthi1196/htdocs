...
if(isset($_POST['file']) && ($_POST['file']=='Upload'))
{
    	...
    	mysqli_query($connect, "update user_profile_pic set image='$img_name' where user_id=$userid;");
...
if(isset($_POST['file1']) && ($_POST['file1']=='Upload'))
{
    	...
    	mysqli_query($connect, "insert into user_cover_pic(user_id,image) values('$userid','$img_name');");
		header("location:Profile.php");
}
...
if(isset($_POST['file2']) && ($_POST['file2']=='Upload'))
{
		...
		mysqli_query($connect, "update user_cover_pic set image='$img_name' where user_id=$userid;");
		header("location:Profile.php");
}
?>
<html>
...
<!--cover img-->
<?php 
	$query3=mysqli_query($connect, "select * from user_cover_pic where user_id=$userid");
	$rec3=mysqli_fetch_array($query3);
	$cover_img=$rec3[2];
	
	$que_post_bg=mysqli_query($connect, "select * from user_post where user_id=$userid");
