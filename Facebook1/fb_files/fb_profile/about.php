...
<?php
	if(isset($_POST['work_sub']))
	{
		$u_job=$_POST['job'];
		$u_edu=$_POST['edu'];
		mysqli_query($connect, "update user_info set job='$u_job',school_or_collage='$u_edu' where user_id=$userid;");
	}
	...
	if(isset($_POST['leving_sub']))
	{
		...
		mysqli_query($connect, "update user_info set  	current_city='$u_city',hometown='$u_hometown' where user_id=$userid;");
	}
	...
	if(isset($_POST['basic_sub']))
	{
		if($_POST['day']=='Day:' && $_POST['month']=='Month:' && $_POST['year']=='Year:')
		{
			...
			mysqli_query($connect, "update user_info set relationship_status='$u_relationship' where user_id=$userid;");
		}
		else
		{
			...
			if(checkdate($month,$day,$year))
			{
				...
				mysqli_query($connect, "update user_info set relationship_status='$u_relationship' where user_id=$userid;");
				mysqli_query($connect, "update users set Birthday_Date='$u_birthday_date' where user_id=$userid;");
			}
			else
			{
				echo "<script>
	...
	if(isset($_POST['contact_sub']))
	{
		...
		$u_web=$_POST['web'];
		$u_fb_id=$_POST['fbid'];
		mysqli_query($connect, "update user_info set mobile_no='$u_m_no',mobile_no_priority='$u_priority',website='$u_web',Facebook_ID='$u_fb_id' where user_id=$userid;");
	}
...
<div style="position:absolute;left:17%; top:139%; height:0.09%; width:66%; background:#CCC; z-index:1">
 ...
 <!--Basic Information--> 
 <?php
	$user_data_query=mysqli_query($connect, "select * from users where Email='$user'");
